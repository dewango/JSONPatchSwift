...
extension JPSJsonPointerTests {
    ...
    func testIfEmptyPointerIsValid() {
        guard let jsonPointer: JPSJsonPointer = try? JPSJsonPointer(rawValue: "") else {
    ...
    func testForSeveralUnicodeCharacters() {
        let rawValue: String = "/1234567890-=!@£$%^&*()_+¡€#¢∞§¶•ªº–≠⁄™‹›ﬁﬂ‡°·‚—±qwertyuiop["
            + "]QWERTYUIOP{}œ∑´®†¥¨^øπ“‘Œ„‰ÂÊÁËÈØ∏’asdfghjkl;'ASDFGHJKL:|åß∂ƒ©˙∆˚¬…æ«ÅÍÎÏÌÓÔÒÚÆ»"
            + "`zxcvbnm,./~ZXCVBNM<>?`Ω≈ç√∫~µ≤≥÷ŸÛÙÇ◊ıˆ˜¯˘¿"
        guard let jsonPointer: JPSJsonPointer = try? JPSJsonPointer(rawValue: rawValue) else {
            XCTFail("json parse error")
            return
        }
...
extension JPSJsonPointerTests {
    ...
    func testIfTildeEscapedCharactersAreDecoded() {
        guard let jsonPointer1: JPSJsonPointer = try? JPSJsonPointer(rawValue: "/~1") else {
            XCTFail("json parse error")
            return
        }
        ...
        guard let jsonPointer2: JPSJsonPointer = try? JPSJsonPointer(rawValue: "/~0") else {
            XCTFail("json parse error")
            return
        }
        ...
        guard let jsonPointer3: JPSJsonPointer = try? JPSJsonPointer(rawValue: "/~01") else {
            XCTFail("json parse error")
            return
        }
        ...
        guard let jsonPointer4: JPSJsonPointer = try? JPSJsonPointer(rawValue: "/~10") else {
            XCTFail("json parse error")
            return
        }
        XCTAssertEqual(jsonPointer4.pointerValue.count, 1)
        XCTAssertEqual(jsonPointer4.pointerValue.first as? String, "/0")
        guard let jsonPointer5: JPSJsonPointer = try? JPSJsonPointer(rawValue: "/~1~0") else {
            XCTFail("json parse error")
            return
        }
        XCTAssertEqual(jsonPointer5.pointerValue.count, 1)
        XCTAssertEqual(jsonPointer5.pointerValue.first as? String, "/~")
        guard let jsonPointer6: JPSJsonPointer = try? JPSJsonPointer(rawValue: "/~1/~0") else {
            XCTFail("json parse error")
            return
        }
        ...
        guard let jsonPointer7: JPSJsonPointer = try? JPSJsonPointer(rawValue: "/~0/~1") else {
            XCTFail("json parse error")
            return
        }
...
extension JPSJsonPointerTests {
    func testIfTraverseReturnsNextPointer() {
        guard let pointer: JPSJsonPointer = try? JPSJsonPointer(rawValue: "/a/b") else {
            XCTFail("json parse error")
            return
        }
        let newPointer: JPSJsonPointer = JPSJsonPointer.traverse(pointer)
        XCTAssertEqual(newPointer.rawValue, "/b")
    }

    func testIfDeepTraverseReturnsNextPointer() {
        guard let pointer: JPSJsonPointer = try? JPSJsonPointer(rawValue: "/abc/b/hallo/welt") else {
            XCTFail("json parse error")
            return
        }
        let newPointer: JPSJsonPointer = JPSJsonPointer.traverse(pointer)
